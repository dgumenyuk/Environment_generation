    ...
    def _run_simulation(self, the_test) -> SimulationData:
        ...
        try:
            ...
            while True:
                ...
                sim_data_collector.collect_current_data(oob_bb=True)
                #throttle =vehicle_state_reader.get_state().vel_kmh
                #print(vehicle_state_reader.get_state().throttle)
        ...
        jerk = diff(throttle_list)/diff(time_list)
        result = []
        ...
        print("JERK", result)
        print("Average acceleration", sum(result)/len(result))
        print("STdev", statistics.stdev(result))
        '''
