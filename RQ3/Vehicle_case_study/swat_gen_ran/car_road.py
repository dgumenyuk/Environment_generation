...
class Map:
    ...
    def point_in_range_2(self, a):
        """check if point is in the acceptable range"""
        if ((0 + 4) < a[0] and a[0] < (self.max_x - 4)) and ((0 +4) < a[1] and a[1] < (self.max_y - 4)):
            return 1
        else:
            return 0
...
if __name__ == "__main__":
    ...
    '''
    cases = {'st0': {'state': 'right', 'value': 65}, 'st1': {'state': 'straight', 'value': 10}, 'st2': {'state': 'left', 'value': 20}, 'st3': {'state': 'right', 'value': 70}}
    my_map = Map(200)
