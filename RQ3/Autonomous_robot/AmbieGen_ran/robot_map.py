...
class Map:
    ...
    def horizontal(self, distance, position):
        ...
        new_points = []
        init_pos = [int(position), int(self.current_level)]
    ...
    def point_valid(self, point):
        ...
        if (
            (point in self.all_map_points)
            or (self.in_polygon(point))
            or (self.point_out_of_bounds(point))
        ):
            return False
        else:
            return True

    def point_out_of_bounds2(self, a):
        if (0 < a[0] and a[0] < self.max_x - 3) and (
            0 < a[1] and a[1] < self.max_y - 3
        ):
            return False
        else:
            return True
    ...
    def in_polygon(self, a):
        ...
        s1 = [self.min_x, self.max_y + s]
        s2 = [self.min_x + s, self.min_y + s]
        s3 = [self.max_x + s, self.max_y]

        point = Point(a[0], a[1])
        polygon1 = Polygon([finish, f1, f2, f3])
        polygon2 = Polygon([start, s1, s2, s3])
    ...
    def get_points_from_states(self, states):
        ...
        for state in tc:
            ...
            if action == "horizontal":
                new_points = self.horizontal(tc[state]["value"], tc[state]["position"])
                self.map_lists.append(new_points)
            elif action == "vertical":
                new_points = self.vertical(tc[state]["value"], tc[state]["position"])
                self.map_lists.append(new_points)
            else:
                print("ERROR")
